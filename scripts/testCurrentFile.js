...
const startTesting = () => {
  ...
  const testFilePath = jestArgs[0];

  if (!(/\.test.ts?$/g).test(testFilePath)) {
    console.log('\n');
    console.log('Invalid Test File :' + testFilePath);
    console.log('File name should end with .test.ts');
    console.log('\n');
    return;
  }
  ...
  if (jestArgs.some(arg => arg.includes("--collectCoverage=true")) && !jestArgs.some(arg => arg.includes("--collectCoverageFrom="))) {
    const collectCoverageFrom = collectCoverageFromPath(testFilePath);
    const collectCoverageFromArg = "--collectCoverageFrom=" + collectCoverageFrom;
    console.log("\n", collectCoverageFromArg);
    jestArgs.unshift(collectCoverageFromArg)
  }
